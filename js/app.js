(function() {
  ...
  app.config(function(hotkeysProvider) {
    ...
    hotkeysProvider.cheatSheetDescription = 'Show/Hidden';
  });
  ...
  app.run(['angularPlayer', 'Notification', 'loWeb', function(angularPlayer, Notification, loWeb) {
    angularPlayer.setBootstrapTrack(
      loWeb.bootstrapTrack(
        function(){}, 
        function(){
          Notification.info('Copyright Need, Search other platforms please');
        })
    );
  }]);
  ...
  app.controller('NavigationController', ['$scope', '$http',
    ...
    $scope.showDialog = function(dialog_type, data) {
      ...
      if (dialog_type == 0) {
        $scope.dialog_title = 'Add to Playlist';
      ...
      if (dialog_type == 2) {
        $scope.dialog_title = 'Douban Login';
        $scope.dialog_type = 2;
      }

      if (dialog_type == 3) {
        $scope.dialog_title = 'Edit Playlist';
        $scope.dialog_type = 3;
        $scope.dialog_cover_img_url = data.cover_img_url;
        $scope.dialog_playlist_title = data.playlist_title;
      }
      if (dialog_type == 4) {
        $scope.dialog_title = 'Last.fm';
        $scope.dialog_type = 4;
      }
      if (dialog_type == 5) {
        $scope.dialog_title = 'Add Playlist';
        $scope.dialog_type = 5;
      }
    };
    ...
    $scope.importMySettings = function(event) {
      ...
      if (fileObject == null ){
        Notification.warning("Select file...");
        return;
      }
      ...
      reader.onloadend = function(readerEvent) {
        if (readerEvent.target.readyState == FileReader.DONE) {
          ...
          if(data == null) {
            Notification.warning("Backup file format error, please reselect");
            return;
          }
          for ( var key in data) {
            var value = data[key];
    ...
    $scope.openUrl = function(url) {
      loWeb.post({
        ...
        if (result !== undefined) {
          ...
          Notification.info('Failed to open address entered');
        }
      });
    }
  ...
  app.controller('PlayController', ['$scope', '$timeout','$log',
    ...
    'loWeb', 'hotkeys', 'lastfm',
     function($scope, $timeout, $log, $anchorScroll, $location,
      ...
      hotkeys.add({
        combo: 'l',
        description: 'Open/Close Playlist',
        callback: function() {
      ...
      hotkeys.add({
        combo: 's',
        description: 'Switching mode(In order/Random/Loop)',
        callback: function() {
